...
$examType = isset($_GET['exam']) ? intval($_GET['exam']) : 0;
if (!$examType) {
...
if ($exam_id) {
    ...
    if (!$exam_json) {
        ...
        log_error( "TEST: ".print_r($exam_json) );
        ...
        if ($exam_json) {
            ...
            $insert_stmt->bind_param("isiii", $class_id, $exam_json, $duration, $total_marks, $examType, $created_by);
            $insert_stmt->execute();
            $insert_stmt->close();
        } else {
