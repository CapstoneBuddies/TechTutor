...
/**
 * This function will grab the average rating for the specified class
 * @param $classId, the class id to be referred RatingManagement average_rating
 */
function getClassRating($classId) {
    global $conn;
    ...
    if (!$result || !isset($result['tutor_id'])) {
        return 0;
    }

    require_once BACKEND.'rating_management.php';
    $ratinManager = new RatingManagement();

    return $ratinManager->getTutorRatingStats($result['tutor_id'])['average_rating'];
}
function enrollStudent($studentId, $classId) {
    ...
    try {
        // Validate required fields
        if (empty($studentId) || !is_numeric($classId)) {
        ...
        try {
            ...
            $attend_stmt = $conn->prepare("INSERT INTO attendance (student_id, schedule_id, status, created_at) 
                                  ...
                                  FROM class_schedule 
                                  WHERE class_id = ?");
            $attend_stmt->bind_param("ii", $studentId, $class_id);
            ...
            sendEnrollmentEmail($student['email'], $student['name'], $class['class_name'], $class['first_name'] . ' ' . $class['last_name']);
