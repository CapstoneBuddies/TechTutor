...
CREATE TABLE `certificate` (
  ...
  `recipient_name` varchar(255) GENERATED ALWAYS AS (CONCAT((SELECT first_name FROM users WHERE uid = recipient),' ',(SELECT last_name FROM users WHERE uid = recipient))) STORED,
  `donor_name` varchar(255) GENERATED ALWAYS AS (CONCAT((SELECT first_name FROM users WHERE uid = donor),' ',(SELECT last_name FROM users WHERE uid = donor))) STORED,
  `class_name` varchar(255) GENERATED ALWAYS AS ((SELECT class_name FROM class WHERE class_id = class_id)) STORED,
  PRIMARY KEY (`cert_id`),
